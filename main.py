...
class DataExporter:
    ...
    def to_csv(data, filename):
        ...
        return f'<a href="data:file/csv;base64,{b64}" download="{filename}">💾 Preuzmi CSV</a>'
    ...
    def to_excel(data, filename):
        ...
        return f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{filename}">📊 Preuzmi Excel</a>'
...
class AnalyticsEngine:
    ...
    def get_event_stats(user_id, date_from, date_to):
        try:
            ...
            st.error(f"Greška pri dohvaćanju statistika: {str(e)}")
            return {"total": 0, "by_area": {}, "by_category": {}, "by_day": {}}
...
def login_page():
    st.title("🔐 Multi-User Event Diary Login")
            ...
            if submitted:
                with st.spinner("Prijavljivanje..."):
                    ...
                    if hasattr(response, 'user') and response.user:
                        ...
                        st.success("Uspješno ste se prijavili!")
                        st.rerun()
                    else:
            ...
            if submitted:
                if password != password_confirm:
                    ...
                    with st.spinner("Registracija..."):
                        ...
                        if hasattr(response, 'user') and response.user:
                            st.success("Uspješno ste se registrirali! Molimo prijavite se.")
                        else:
                            st.error(f"Greška pri registraciji: {response.get('error', 'Nepoznata greška')}")
...
def navigation_sidebar():
    ...
    pages = {
        ...
        "bulk_add": "📝 Bulk dodavanje",
        "analytics": "📊 Analytics",
        "calendar": "🗓️ Kalendar",
        "export": "💾 Export podataka"
    }
...
def main_dashboard():
    st.title("📅 Event Diary Dashboard")
    ...
    # Text search
    search_text = st.text_input("🔍 Pretraži po komentaru:", value=st.session_state.search_text)
    ...
    try:
        ...
        st.error(f"Greška pri dohvaćanju događaja: {str(e)}")
        events = []

    # Quick stats
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Ukupno događaja", len(events))
    with col2:
    ...
    with col4:
        if events:
            ...
            days_since = "N/A"
        st.metric("Zadnji događaj prije", f"{days_since} dana" if isinstance(days_since, int) else days_since)

    # Display events with pagination
    st.subheader(f"📋 Događaji")
    ...
    if page_events:
        for event in page_events:
            with st.expander(f"{event['occurred_at'][:16]} - {event['mu_category']['name'] if event['mu_category'] else 'N/A'}"):
                ...
                with col1:
                    st.write(f"**Područje:** {event['mu_category']['mu_area']['name'] if event['mu_category'] and event['mu_category']['mu_area'] else 'N/A'}")
        ...
        st.info("Nema događaja prema odabranim filtrima.")

def add_event_page():
    st.title("➕ Dodaj novi događaj")
    ...
    with st.form("add_event_form"):
        ...
        with col1:
            ...
            if area_options:
                ...
                st.error("Nema dostupnih područja. Molimo dodajte područje putem Supabase interface.")
                return
        ...
        with col2:
            ...
            if filtered_categories:
                ...
                st.warning("Nema kategorija za odabrano područje")
        ...
        submitted = st.form_submit_button("💾 Spremi događaj", type="primary")
        ...
        if submitted and selected_category_id:
            try:
                ...
                with st.spinner("Spremanje događaja..."):
                ...
                if response.data:
                    st.success("Događaj je uspješno dodan!")
                    # Clear form
                    st.rerun()
                else:
                    st.error("Greška pri dodavanju događaja")
...
def edit_event_page():
    if 'edit_event_id' not in st.session_state:
        st.error("Nema događaja za uređivanje")
        return

    event_id = st.session_state.edit_event_id

    st.title("✏️ Uredi događaj")
    ...
    try:
        ...
        if not event:
            st.error("Događaj nije pronađen")
            return
    except Exception as e:
        st.error(f"Greška pri dohvaćanju događaja: {str(e)}")
        return
    ...
    with st.form("edit_event_form"):
        ...
        with col_save:
            submitted = st.form_submit_button("💾 Spremi promjene", type="primary")
        with col_cancel:
        ...
        if submitted:
            try:
                ...
                if response.data:
                    st.success("Događaj je uspješno ažuriran!")
                    if 'edit_event_id' in st.session_state:
                        del st.session_state['edit_event_id']
                    st.session_state.current_page = 'dashboard'
                    st.rerun()
                else:
                    st.error("Greška pri ažuriranju događaja")
            except json.JSONDecodeError:
                st.error("Neispravni JSON format u dodatnim podacima")
            except Exception as e:
                st.error(f"Greška: {str(e)}")
...
def bulk_add_page():
    st.title("📝 Bulk dodavanje događaja")

    st.info("Dodajte više događaja odjednom koristeći CSV format ili manual unos.")

    tab1, tab2 = st.tabs(["📊 CSV Upload", "✍️ Manual unos"])
    ...
    with tab1:
        ...
        with st.expander("📖 Prikaz CSV formata"):
            example_df = pd.DataFrame({
                ...
                'comment': ['Prvi događaj', 'Drugi događaj'],
                'data': ['{}', '{"tip": "meeting"}']
            })
        ...
        if uploaded_file:
            try:
                ...
                st.write(f"Učitan CSV s {len(df)} redaka")
                st.dataframe(df.head())

                if st.button("📥 Dodaj sve događaje iz CSV-a"):
                    events_to_add = []
                    ...
                    with st.spinner("Dodavanje događaja..."):
                        result = BulkOperations.bulk_add_events(events_to_add, st.session_state.user_id)

                    if result.get('success'):
                        st.success(f"Uspješno dodano {result['count']} događaja!")
                    else:
                        st.error(f"Greška: {result.get('error')}")

            except Exception as e:
                st.error(f"Greška pri obradi CSV-a: {str(e)}")

    with tab2:
        st.subheader("Manual unos više događaja")

        # Number of events to add
        num_events = st.number_input("Broj događaja za dodavanje:", min_value=1, max_value=20, value=3)

        events_data = []

        for i in range(num_events):
            st.write(f"### Događaj {i+1}")
            col1, col2, col3 = st.columns(3)

            with col1:
                area_options = {area['id']: area['name'] for area in areas}
                selected_area = st.selectbox(f"Područje {i+1}:", list(area_options.keys()), format_func=lambda x: area_options[x], key=f"area_{i}")
        ...
        if st.button("📝 Dodaj sve događaje"):
            with st.spinner("Dodavanje događaja..."):
...
def analytics_page():
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Ukupno događaja", stats['total'])
    with col2:
    ...
    with col4:
        ...
        st.metric("Najaktivnije područje", most_active_area)
    ...
    if stats['total'] > 0:
        ...
        with col1:
            st.subheader("Događaji po područjima")
            if stats['by_area']:
                fig = px.pie(
                    values=list(stats['by_area'].values()),
                    names=list(stats['by_area'].keys()),
                    title="Distribucija po područjima"
                )
                st.plotly_chart(fig, use_container_width=True)

        with col2:
            st.subheader("Događaji po kategorijama")
            if stats['by_category']:
                fig = px.bar(
                    x=list(stats['by_category'].keys()),
                    y=list(stats['by_category'].values()),
                    title="Broj događaja po kategorijama"
                )
        ...
        if stats['by_day']:
            ...
            fig = px.line(
                x=dates,
                y=counts,
                title="Broj događaja po danima",
                markers=True
            )
            fig.update_layout(xaxis_title="Datum", yaxis_title="Broj događaja")
            st.plotly_chart(fig, use_container_width=True)
...
def calendar_view_page():
    ...
    # Calendar grid
    days_of_week = ['Pon', 'Uto', 'Sri', 'Čet', 'Pet', 'Sub', 'Ned']
    ...
    if events:
        st.subheader(f"📋 Svi događaji u {calendar.month_name[selected_month]}")
...
def export_page():
    ...
    with col2:
        date_range = st.selectbox("Raspon:", ["Zadnjih 30 dana", "Zadnjih 90 dana", "Ova godina", "Sve podatke", "Prilagođeno"])

    # Date range selection
    if date_range == "Prilagođeno":
        col1, col2 = st.columns(2)
        with col1:
    ...
    if events:
        st.write(f"📊 Pronađeno {len(events)} događaja za export")
        ...
        for event in events:
            export_data.append({
                ...
                'Područje': event['mu_category']['mu_area']['name'] if event['mu_category'] and event['mu_category']['mu_area'] else 'N/A',
        ...
        # Export buttons
        st.subheader("⬇️ Preuzimanje")

        col1, col2, col3 = st.columns(3)

        with col1:
            if export_format in ["CSV", "Excel"]:
                if st.button("📥 Preuzmi CSV"):
                    csv = df.to_csv(index=False)
                    st.download_button(
                        label="💾 Preuzmi CSV datoteku",
                        data=csv,
